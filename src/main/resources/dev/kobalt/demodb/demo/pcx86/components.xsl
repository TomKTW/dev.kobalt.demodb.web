...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="SITEURL">/external?url=https://www.pcjs.org</xsl:variable>
		...
		<div id="{$id}" class="{$componentClass}" style="{$width}{$height}{$pos}{$left}{$top}{$padding}" data-value="{$machineParms}">
				...
				<xsl:if test="$class != '' and $component != 'machine'">
					<div class="{$objectClass}" data-value="{{id:'{$id}',name:'{$name}'{$comment}{$parms}}}">
						<xsl:if test="$class = 'video'">
							<canvas class="pcjs-canvas" width="{$screenWidth}" height="{$screenHeight}" style="height: auto; background-color: black;"/>
						</xsl:if>
					</div>
				</xsl:if>
				<xsl:if test="control">
			...
			<xsl:if test="$component = 'machine'">
				...
				<!--<div class="{$CSSCLASS}-copyright">
					<a href="{$SITEURL}" target="_blank"><xsl:value-of select="$APPNAME"/></a>&#160;<xsl:value-of select="$APPVERSION"/> © 2012-2020 (<a href="/external?url=https://www.pcjs.org/LICENSE.txt" target="_blank">MIT</a>) by Jeff Parsons (<a href="/external?url=https://jeffpar.com" target="_blank">@jeffpar</a>)
				</div>
